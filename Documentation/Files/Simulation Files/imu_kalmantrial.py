...
from kalmanClass import KalmanAngle
...
while True:
    ...
    print("Gyroscope\tX: %0.6f\tY: %0.6f\tZ: %0.6f\trads/s" % (gyroX, gyroY, gyroZ))
    display.text("Gyr: X:{:0.2f},Y:{:0.2f},Z:{:0.2f}".format(gyroX, gyroY, gyroZ ), 0, 12, scale=0.25			)
    ...
    display.text("Angle X: :{:s},Angle Y:{:s}".format(kalAngleX, kalAngleY), 0, 24, scale=0.25)
    #.text("Roll{:0.2f}  Angle {:0.2f},Z:{:0.2f}".format(kalAngleX, kalAngleY), 0, 24, scale=0.25)
    time.sleep(0.005)
